...
class TaxonomyValidator(IValidator):
    ...
    def validate(self, data: Dict[str, Any]):
        ...
        def procesar_taxonomia(biotico: pd.DataFrame) -> dict:
            ...
            # try:
            #     for ii in lista_argumentos:
            #         print(f"--processing {ii}--")
            #         taxonomy_validation(ii)
            # except Exception as e:
            #     print(e)
        ...
        def replace_strings_with_nan(x):
            try:
                return float(x)
            except ValueError:
                return np.nan
        ...
        def taxonomy_revision(fila_path: str) -> pd.DataFrame:
            ...
            biotico["CORREL"] = biotico["CORREL"].apply(
                lambda x: x if isinstance(x, float) else 0
            )

            return biotico

        result = taxonomy_revision(data)
